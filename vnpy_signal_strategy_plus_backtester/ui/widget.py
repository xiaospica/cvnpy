...
from ..engine import (
    ...
    OptimizationSetting,
    SignalBacktesterEngine
)


class SignalBacktesterManager(QtWidgets.QWidget):
    """"""

    setting_filename: str = "signal_backtester_setting.json"
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine) -> None:
        ...
        self.backtester_engine: SignalBacktesterEngine = main_engine.get_engine(APP_NAME)     # type: ignore
    ...
    def init_ui(self) -> None:
        """"""
        self.setWindowTitle(_("Signal回测"))
        ...
        # Result part
        self.statistics_monitor: SignalBacktesterStatisticsMonitor = SignalBacktesterStatisticsMonitor()

        self.log_monitor: QtWidgets.QTextEdit = QtWidgets.QTextEdit()

        self.chart: SignalBacktesterChart = SignalBacktesterChart()
        chart: QtWidgets.QVBoxLayout = QtWidgets.QVBoxLayout()
        chart.addWidget(self.chart)

        self.trade_dialog: SignalBacktestingResultDialog = SignalBacktestingResultDialog(
            self.main_engine,
            self.event_engine,
            _("回测成交记录"),
            SignalBacktestingTradeMonitor
        )
        self.order_dialog: SignalBacktestingResultDialog = SignalBacktestingResultDialog(
            self.main_engine,
            self.event_engine,
            _("回测委托记录"),
            SignalBacktestingOrderMonitor
        )
        self.daily_dialog: SignalBacktestingResultDialog = SignalBacktestingResultDialog(
            self.main_engine,
            self.event_engine,
            _("回测每日盈亏"),
            SignalBacktestingDailyResultMonitor
        )

        # Candle Chart
        self.candle_dialog: SignalBacktesterCandleChartDialog = SignalBacktesterCandleChartDialog()
    ...
    def start_backtesting(self) -> None:
        ...
        dialog: SignalBacktestingSettingEditor = SignalBacktestingSettingEditor(class_name, old_setting)
        i: int = dialog.exec()
    ...
    def start_optimization(self) -> None:
        ...
        dialog: SignalBacktesterOptimizationSettingEditor = SignalBacktesterOptimizationSettingEditor(class_name, parameters)
        i: int = dialog.exec()
        if i != dialog.DialogCode.Accepted:
            return
    ...
    def show_optimization_result(self) -> None:
        ...
        dialog: SignalBacktesterOptimizationResultMonitor = SignalBacktesterOptimizationResultMonitor(
            result_values,
            self.target_display
        )
        dialog.exec_()
...
class SignalBacktesterStatisticsMonitor(QtWidgets.QTableWidget):
    """"""
    KEY_NAME_MAP: dict = {
        "start_date": _("首个交易日"),
        "end_date": _("最后交易日"),
...
class SignalBacktestingSettingEditor(QtWidgets.QDialog):
    """
...
class SignalBacktesterChart(pg.GraphicsLayoutWidget):
    """"""

    def __init__(self) -> None:
        """"""
        super().__init__(title="SignalBacktester Chart")
    ...
    def init_ui(self) -> None:
        ...
        self.balance_plot = self.addPlot(
            title=_("账户净值"),
            axisItems={"bottom": SignalBacktesterDateAxis(self.dates, orientation="bottom")}
        )
        self.nextRow()
...
class SignalBacktesterDateAxis(pg.AxisItem):
...
class SignalBacktesterOptimizationSettingEditor(QtWidgets.QDialog):
    """
...
class SignalBacktesterOptimizationResultMonitor(QtWidgets.QDialog):
    """
...
class SignalBacktestingTradeMonitor(BaseMonitor):
    """
...
class SignalBacktestingOrderMonitor(BaseMonitor):
    """
...
class SignalBacktesterFloatCell(BaseCell):
    """
...
class SignalBacktestingDailyResultMonitor(BaseMonitor):
    """
    ...
    headers: dict = {
        ...
        "turnover": {"display": _("成交额"), "cell": SignalBacktesterFloatCell, "update": False},
        "commission": {"display": _("手续费"), "cell": SignalBacktesterFloatCell, "update": False},
        "slippage": {"display": _("滑点"), "cell": SignalBacktesterFloatCell, "update": False},
        "trading_pnl": {"display": _("交易盈亏"), "cell": SignalBacktesterFloatCell, "update": False},
        "holding_pnl": {"display": _("持仓盈亏"), "cell": SignalBacktesterFloatCell, "update": False},
        "total_pnl": {"display": _("总盈亏"), "cell": SignalBacktesterFloatCell, "update": False},
        "net_pnl": {"display": _("净盈亏"), "cell": SignalBacktesterFloatCell, "update": False},
    }


class SignalBacktestingResultDialog(QtWidgets.QDialog):
    """"""
...
class SignalBacktesterCandleChartDialog(QtWidgets.QDialog):
    """"""
