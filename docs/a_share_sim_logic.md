...
**✅ 已实现的优化**：
- **先卖后买等待机制**：策略现在能感知活跃卖单（`_active_sell_orders`），当存在未终态卖单时，买入信号会被自动延迟执行（`_defer_buy_until`），从而避免 T+1 制度下卖出未成交占用资金导致的“买单资金不足拒单”问题。
- **资金不足延时重挂**：策略引入了自动重挂混合类（`AutoResubmitMixinPlus`），专门识别“可用资金不足”拒单，并在设定退避时间后进行延时重试，避免拒单死循环。
