...
class XtGateway(BaseGateway):
    ...
    def _connect(self, setting: dict) -> None:
        ...
        self.timeout_cancel_enabled = setting.get("是否启用超时撤单", "否") == "是"
    ...
    def get_full_tick(self, vt_symbol: str) -> TickData | None:
        return self.md_api.get_full_tick(vt_symbol)
    ...
    def on_order(self, order: OrderData) -> None:
        ...
        if not order.is_active():
            ...
            self._cancel_sent.discard(order.orderid)
    ...
    def check_order_timeout(self) -> None:
        ...
        for orderid, submit_time in list(self.order_submit_time.items()):
            ...
            if orderid not in self.td_api.active_localid_sysid_map:
                continue
...
class XtMdApi:
    ...
    def _convert_xt_tick(self, xt_symbol: str, d: dict) -> TickData | None:
        try:
        ...
        bp_data: list = d.get("bidPrice") or [0, 0, 0, 0, 0]
        ap_data: list = d.get("askPrice") or [0, 0, 0, 0, 0]
        bv_data: list = d.get("bidVol") or [0, 0, 0, 0, 0]
        av_data: list = d.get("askVol") or [0, 0, 0, 0, 0]
        ...
        tick.last_price = round_to(d.get("lastPrice", 0), contract.pricetick)
        ...
        elif d.get("settlementPrice", 0) > 0:
    ...
    def onMarketData(self, data: dict) -> None:
        ...
        for xt_symbol, buf in data.items():
            for d in buf:
                tick = self._convert_xt_tick(xt_symbol, d)
                if tick:
                    self.gateway.on_tick(tick)

    def get_full_tick(self, vt_symbol: str) -> TickData | None:
        if vt_symbol not in symbol_contract_map:
            return None
        ...
        xt_exchange: str = EXCHANGE_VT2XT.get(exchange, "")
        if not xt_exchange:
            return None

        if xt_exchange in {"SH", "SZ"} and len(symbol) > 6:
            xt_exchange += "O"

        xt_symbol = f"{symbol}.{xt_exchange}"
        ...
        try:
            ...
            self.gateway.write_log(f"主动获取五档行情失败: {vt_symbol} {e}\n{traceback.format_exc()}")
            return None

        data = None
        if isinstance(raw, dict):
            data = raw.get(xt_symbol)
            if data is None and len(raw) == 1:
        ...
        return self._convert_xt_tick(xt_symbol, data)
...
class XtTdApi(XtQuantTraderCallback):
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        self.gateway.order_submit_time[orderid] = datetime.now()
