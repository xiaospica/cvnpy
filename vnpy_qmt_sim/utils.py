...
def to_vn_contract(symbol):
    result = parse_symbol_exchange(symbol)
    if not result:
        raise ValueError(f"无法解析合约代码: {symbol}")
    return result


def parse_symbol_exchange(code: str) -> tuple[str, Exchange] | None:
    ...
    if suffix in {"SHO", "SZO"}:
        return symbol, Exchange.SSE if suffix == "SHO" else Exchange.SZSE
