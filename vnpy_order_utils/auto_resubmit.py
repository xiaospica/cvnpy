"""策略层撤单/拒单重挂 mixin — 跨 app 共享.

原位于 ``vnpy_signal_strategy_plus/auto_resubmit.py`` 的 AutoResubmitMixinPlus,
抽到 vnpy_order_utils 后:
  * 主类改名 ``AutoResubmitMixin`` (去掉 Plus 后缀, 这里它不是 Plus 的特化)
  * ``vnpy_signal_strategy_plus.auto_resubmit`` 留 re-export 保留
    ``AutoResubmitMixinPlus`` 类别名, 现有 import 零破坏

Host 契约见 ``protocols.OrderResubmitHost``.

重挂策略:
  - CANCELLED: 立即挂(ready_at = now), ``resubmit_clock`` 按 ``resubmit_interval``
    节流; 上限 ``resubmit_limit``
  - REJECTED + direction=LONG + "资金不足"关键字: 指数退避
    (delay × 2^attempts, 封顶 ``reject_resubmit_backoff_max_seconds``),
    上限 ``reject_resubmit_limit``
  - 价格重算: 走 pricing.choose_order_price 读最新 tick 的对手一档

外部可观察状态(子类可读取, 是 API 一部分):
  - ``self._is_resubmitting`` — bool, 重挂发单期间 True.
    参考 vnpy_signal_strategy_plus/strategies/live_order_test_strategy.py:44
    的 get_order_reference 消费.
"""
...
from .pricing import choose_order_price


class AutoResubmitMixin:
    """策略层撤单/拒单重挂混入能力."""
