...
常驻进程：实时消费 Redis Stream 上的下单信号，写入 MySQL `trade_signal_events` 表，供
...
[策略发布端]                  [bridge 进程]                  [vnpy 主进程]
  生产策略    --xadd-->  Redis Stream  --xreadgroup-->  bridge  --INSERT-->  MySQL trade_signal_events
                              ^                              |
                              |<------- xack(成功) ---------|
                              ↓ 失败留 PEL 重试
                                                                                ↑
                                                 [vnpy 主进程] mysql_signal_strategy_plus
                                                                每 50ms 轮询 trade_signal_events
                                                                未被 strategy_signal_applications 消费 AND stg=策略名
...
### 7.5 trade_signal_events 表 schema 漂移
