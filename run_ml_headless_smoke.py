...
参数 ``SMOKE_REPLAY_END_OFFSET = N`` (默认 1):

  - 回放段: 各策略 ``replay_start_date`` → ``today - N`` (含),
    所有交易日逐日推进 (再被 SMOKE_REPLAY_DAYS_LIMIT 截尾)
  - 实时段: ``today - N + 1`` → 之后 ``SMOKE_LIVE_DAYS`` 个交易日,
    每日尾插 ``SMOKE_LIVE_SECONDS_PER_DAY`` sleep 让 mlearnweb snapshot loop
    与 webtrader REST 能采到当日数据

两段无缝衔接, 同一日序列循环, 仅 phase 标签不同. N=1 (默认) 即"回放到昨天,
今天开始定时模拟", 对齐生产实时启动语义.
...
# 缩短回放跨度冒烟
SMOKE_REPLAY_DAYS_LIMIT=2 SMOKE_LIVE_DAYS=1 \\
    F:/Program_Home/vnpy/python.exe -u run_ml_headless_smoke.py
...
# --- 1) 阶段切分 ---
# N=1 (默认): 回放到 today-1, 实时段从 today 起 (生产实时启动语义)
# N=0:        回放到 today (含), 实时段从 today+1 起
# N=2,3,...:  扩大实时段验证 (回放停得更早)
SMOKE_REPLAY_END_OFFSET: int = int(os.getenv("SMOKE_REPLAY_END_OFFSET", "1"))

# 回放段交易日上限 (从 replay 起点起取最近 N 个), 0=不限
SMOKE_REPLAY_DAYS_LIMIT: int = int(os.getenv("SMOKE_REPLAY_DAYS_LIMIT", "5"))
...
def main() -> int:
    ...
    today = date.today()
    boundary = today - timedelta(days=SMOKE_REPLAY_END_OFFSET)
    _log(f"  today={today} REPLAY_END_OFFSET={SMOKE_REPLAY_END_OFFSET} → boundary={boundary}")
    ...
    replay_days = build_trade_days(replay_start, boundary, _is_trade_date_fn)
    if SMOKE_REPLAY_DAYS_LIMIT > 0 and len(replay_days) > SMOKE_REPLAY_DAYS_LIMIT:
        replay_days = replay_days[-SMOKE_REPLAY_DAYS_LIMIT:]
        _log(f"  回放段截尾到最近 {SMOKE_REPLAY_DAYS_LIMIT} 个交易日")

    live_start = boundary + timedelta(days=1)
    live_days = build_next_n_trade_days(live_start, SMOKE_LIVE_DAYS, _is_trade_date_fn)
    ...
    _log(f"PASS: {len(days)} 日 × {len(started)} 策略 全部断言通过 ✓")
