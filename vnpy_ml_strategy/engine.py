...
class MLEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        self._eod_equity_snapshot_keys: set[str] = set()
    ...
    def _on_timer(self, event: Event) -> None:
        ...
        try:
            self._persist_sim_live_eod_equity_from_settled_gateways()
            self._persist_broker_live_eod_equity_after_close()
        except Exception as exc:  # noqa: BLE001
            logger.warning("[MLEngine] EOD equity journal timer failed: %s", exc)
    ...
    def _write_eod_equity_snapshot(
        ...
        *,
        ...
        if ok:
            logger.info(
                "[MLEngine][{}] EOD equity journal persisted source={} day={} equity={:.2f}",
    ...
    def _persist_sim_live_eod_equity_from_settled_gateways(self) -> None:
        ...
        for gateway_name in gateway_names:
            ...
            for strat in self._running_strategies_for_gateway(gateway_name):
                ...
                if key in self._eod_equity_snapshot_keys:
                    continue
                if self._write_eod_equity_snapshot(
                    ...
                    self._eod_equity_snapshot_keys.add(key)

    def _persist_broker_live_eod_equity_after_close(self) -> None:
